...
const SetPostContent = {
  ...
  extractContent (text) {
    ...
    if (text.trim().match(/^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/is) !== null) {
      const urlData = SetPostContent.extractURLData(text)
    ...
    } else if (text.trim().match(/<iframe(.+)<\/iframe>/g) !== null) {
      console.log('<iframe>')
      ...
      const urlData = SetPostContent.extractURLData(src)
