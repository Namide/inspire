...
const getStyle = data =>
{
    return getImg(data) ? { 'backgroundColor': getColor(data) } : {}
}
...
{
    if (getImg(data))
        return <div style={ 'background-image: url(' + data.thumb.src + ')' } class="post-grid_thumb"></div>
    else if (isEmbed(data))
    {
        const size = getOriginalSize(data)
        return <div innerHTML={ data.content.raw.trim() } class="post-grid_embed"></div>
        // <div class="embed">
    ...
    {
        return <a href={ data.content.url } target="_blank" rel="nofollow noopener noreferrer" class="post-grid_url">{ data.content.url.replace(/(^\w+:|^)\/\//, '') }</a>
    }
}

export default ({ onOpen = null, data, observerSubscribe, observerUnsubscribe }) => (state, actions) =>
{
    const valueObject = { }
    ...
    return (
        <button // href={ getHref(data) }
            onclick={ () => onOpen(data) }
            target="blank"
            class={ 'post-grid ' + getClassList(data) }
            style={ getStyle(data) }
            ...
            <h1 class="post-grid_title">
                { data.title }
            </h1>

            <time class="post-grid_date">
                { data.date }
            </time>

            <p class="post-grid_description">
                { data.description }
            </p>

            <ul class="post-grid_tags">
                { data.tags && data.tags.map(tag => (
                    <li class="post-grid_tag">
                        { tag }
                    </li>
                )) }
            </ul>

            <span class="post-grid_score">
                { data.score }/5
            </span>
